# 실험용 수정 로그 (`experiments/yongseong/`)

이 폴더는 **실험 4/5 전용 공간**이다.

## 철학

1저자 core 코드(`IEC/mainddpm/`, `IEC/siec_core/`, `IEC/quant/`, `IEC/mainldm/`)는 **한 글자도 수정하지 않는다**. 수정이 필요한 경우 해당 파일을 이 폴더에 복사한 뒤 복사본만 수정한다. 따라서 본 폴더 바깥의 변경은 어느 경우에도 발생하지 않는다.
